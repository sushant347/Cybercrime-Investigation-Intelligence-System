"""Synthetic case builder shared by both analysis engines' test suites.

Builds one fully synthetic case (``CASE_9001``) inside a temp storage tree,
mirroring every real storage format the data gateway reads: evidence.csv,
...
This lives in the package rather than in ``tests/`` because the timeline &
report engine needs the identical fixture case, and two copies would drift.
"""
...
def seed_case(config: EvidenceConfig, icfg: InvestigationConfig) -> None:
