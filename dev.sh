...
#                      (no-op on builds with no database)
...
setup() {
  ...
  echo ">> [4/4] database migrate + seed demo users (skipped if no database)"
  reset_db
...
reset_db() {
  ...
  if [ ! -d "ciis_api/api/migrations" ]; then
    echo ">> No platform database in this build - nothing to migrate or seed."
    echo "   Case data lives in evidence_ocr_engine/storage/ and is managed"
    echo "   from the app's admin screen."
    return 0
  fi
...
doctor() {
  ...
  echo "  platform DB      : $([ -d ciis_api/api/migrations ] && echo "$([ -f ciis_api/ciis_platform.sqlite3 ] && echo present || echo MISSING)" || echo "n/a (no database in this build)")"
